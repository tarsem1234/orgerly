...
use App\Http\Requests\Frontend\RegisterRequestSigner;
        ...
        if ($data['user_type'] == config('constant.user_type.2') || $data['user_type'] == config('constant.user_type.3')) {
            ...
            if ($user->save()) {
                ...
                if ($data['user_type'] == config('constant.user_type.3')) {
                    ...
                    $userProfile->company_name = $data['company_name'];
                    ...
                    if ($userProfile->save()) {
                        ...
                        $user->notify(new UserNeedsConfirmation($user->confirmation_code, $userProfile->full_name));
        ...
        if (User::where('id', Auth::id())->update($input)) {
            if ($data['user_type'] == config('constant.user_type.3')) {
                ...
                if (! $loggedInUser->user_profile) {
                    ...
                    $userInput['company_name'] = $data['company_name'];
