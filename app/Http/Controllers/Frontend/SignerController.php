...
class SignerController extends Controller
{
    ...
    private function _createUser($request, $ifNewUser)
    {
        ...
        $user->state_id = ! empty($checkState['id']) ? $checkState['id'] : 0;
        ...
        $user->phone_no = ! empty($request->phone_no) ? $request->phone_no : 000000000;
        $user->confirmation_code = md5(uniqid(mt_rand(), true));
        $user->signer_status = 1;
        if ($user->save()) {
    ...
    public function signerView($id)
    {
        if (Auth::check() && $id) {
            ...
            $user = User::where('id', $id)->with('lerning_points')
                ...
                }, 'business_profile'])->withTrashed()->first();
            $network = Network::where('from_user_id', Auth::id())->where('to_user_id', $id)->withTrashed()->first();
