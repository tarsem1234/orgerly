...
@section('title', app_name() . ' | Wellness Center')
@section('after-styles')
<link type="text/css" rel="stylesheet" href="{{ asset(mix('css/dashboard.css')) }}" media="all">
<style>#wellness_center{ font-weight: bold;color: #000;}</style>
@endsection 
@section('content')
            ...
            <div class="col-md-9 col-sm-8">
                <div class="right-dashboard-div" id="quiz-box">
                    <div class="profile right-dashboard-div-text">
                        <div class="row">  
                            <div class="col-md-12">
                                <h4 class="pull-left m-bottom">Wellness Session: {{$session->name}}</h4> 
                            </div>
                        </div>
                        <hr class="lc_hr" />
                                ...
                                <div class="right-dashboard-div-text">
                                    ...
                                    <section v-else>
                                        <div class="btns-green-blue">
                                            ...
                                            @if($isBonusSession)
                                            <a href="{{route('frontend.wellness.center')}}" class="btn btn-blue pull-right">Return</a>
                                            @else
                                            <a href="{{route('frontend.wellness.topic', $session->category_id)}}" class="btn btn-blue pull-right">Continue</a>
                                            @endif
                                        </div>
                                    </section>
                                </div>
    ...
    var quiz = new Vue({
        ...
        methods: {
            ...
            submitAnswer() {
                ...
                axios.post("{{route('frontend.wellness.answer')}}", dataToBePosted).then(function (res) {
                    result = res.data;
