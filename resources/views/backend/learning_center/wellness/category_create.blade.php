...
@if( isset($category) )
@section ('title', ('Edit Wellness Category'))
@else
@section ('title', ('Create Wellness Category'))
@endif
...
@section('page-header')
<h1>{{isset($category)?"Edit Wellness Category":"Add New Wellness Category"}}</h1>
@endsection
...
<div class="box box-success" id="edit-add-pages">
   <div class="box-header with-border">
      @if( isset($category) )
      <h3 class="box-title">Edit Wellness Category</h3>
      @else
      <h3 class="box-title">Create Wellness Category</h3>
      @endif
   ...
   <div class="box-body">
      <div class="container">
         @if( isset($category) )
         {{ html()->modelForm($category, 'PUT', route('admin.wellness_categories.update', $category['id']))->class('form-horizontal')->acceptsFiles()->open() }}
         @else
         {{ html()->form('POST', route('admin.wellness_categories.store'))->class('form-horizontal')->acceptsFiles()->open() }}
         @endif
...
<div class="box box-info  create-edit-cancel-btn">
   ...
   @endif
   <a href="{{route("admin.wellness_categories.index")}}" class="btn btn-primary cancel-btn">Cancel</a>
</div>
{{ html()->closeModelForm() }}
